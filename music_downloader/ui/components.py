...
class SearchTab(QWidget):
    ...
    def _on_results(self, results: List[dict]):
        ...
        if not results:
            ...
            subtitle = QLabel("Try a different search term or check the URL")
            ...
            self.grid_layout.addWidget(empty_widget, 0, 0, 1, 4, Qt.AlignmentFlag.AlignCenter)
            return
...
class SettingsTab(QWidget):
    ...
    def __init__(self, config: Dict):
        ...
        self.concurrent.setCurrentText(str(self._config.get("concurrent_downloads", 16)))
        perf_layout.addWidget(concurrent_row["label"])
        perf_layout.addWidget(self.concurrent)
        v.addWidget(perf_group)
        
        # Appearance Section
        appearance_group = self._create_group("Appearance")
        appearance_layout = QVBoxLayout(appearance_group)
        appearance_layout.setSpacing(15)
        
        theme_row = self._create_setting_row("Theme", "dark")
        self.theme_box = theme_row["widget"]
        self.theme_box.addItems(["Dark", "Light", "AMOLED"])
        current_theme = self._config.get("theme", "dark").lower()
        theme_map = {"dark": "Dark", "light": "Light", "amoled": "AMOLED"}
        self.theme_box.setCurrentText(theme_map.get(current_theme, "Dark"))
